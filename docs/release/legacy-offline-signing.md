# Legacy offline release signing runbook

Historical reference for keyed releases and explicit `--offline` operations.
The current stable workflow uses the [online ceremony](online-signing.md); it
does not build the unsigned drafts described below. Do not mix the two flows.
...
`scripts/release/ceremony.sh --offline` guides the same locked ceremony as five resumable
...
```sh
./scripts/release/ceremony.sh --offline --dry-run
```
...
`nightly/v1` policy. Follow [signed nightly bootstrap and legacy bridges](https://github.com/Hikyo-Org/Hikyo/blob/main/docs/operations/signed-nightlies.md)
