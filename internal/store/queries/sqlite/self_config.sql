...
SELECT 1,i.identity,sqlc.arg(adoption_key),sqlc.arg(adopted_by),s.org_id,s.project_id,s.environment_id,sqlc.arg(schema_version),1,s.revision,s.id,sqlc.arg(incarnation),0,sqlc.arg(created_at),sqlc.arg(created_at)
FROM snapshots s JOIN instance_identity i ON i.id=1
WHERE s.id=sqlc.arg(snapshot_id) AND s.org_id=sqlc.arg(org_id) AND s.project_id=sqlc.arg(project_id) AND s.environment_id=sqlc.arg(environment_id) AND i.identity=sqlc.arg(owner_instance_id) AND s.payload_present=1;
-- hikyo:instance-scoped
...
WHERE b.id=1 AND s.id=sqlc.arg(snapshot_id) AND s.revision=sqlc.arg(revision) AND s.payload_present=1 AND b.generation=sqlc.arg(expected_generation) AND b.schema_version=sqlc.arg(schema_version);
-- hikyo:instance-scoped
...
UPDATE self_config_binding SET desired_snapshot_id=sqlc.arg(snapshot_id),desired_revision=sqlc.arg(revision),generation=generation+1,suspended=0,updated_at=sqlc.arg(updated_at)
...
UPDATE self_config_binding SET incarnation=sqlc.arg(incarnation),suspended=1,updated_at=sqlc.arg(updated_at) WHERE id=1 AND incarnation<>sqlc.arg(incarnation);
-- hikyo:instance-scoped
...
WHERE id=1 AND generation=sqlc.arg(expected_generation)
AND EXISTS(SELECT 1 FROM snapshots s WHERE s.id=sqlc.arg(snapshot_id) AND s.org_id=self_config_binding.org_id AND s.project_id=self_config_binding.project_id AND s.environment_id=self_config_binding.environment_id AND s.revision=sqlc.arg(revision) AND s.payload_present=1);
