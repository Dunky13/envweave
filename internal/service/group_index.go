...
func (i *groupIndex) validateResolvedPublish(cells []resolvedCell, envID string, contract parameters.Contract) error {
	...
	for _, cell := range cells {
		...
		if cell.set {
			if err := validateValueWithContract(cell.key, cell.value, contract); err != nil {
				return err
			}
		}
		if cell.key.GroupID != "" {
